...
function getTime(date) {
  const toStr = (num) => String(num).padStart(2, '0');
  return `${toStr(date.getHours())}:${toStr(date.getMinutes())}:${toStr(date.getSeconds())}`;
}
...
function getCountDaysInMonth(month, year) {
  ...
  const daysInMilliseconds =
    new Date(innerYear, nextMonth) - new Date(year, curMonth);

  return daysInMilliseconds / 86400000;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return 1 + (new Date(dateEnd) - new Date(dateStart)) / 86400000;
}
...
function getWeekNumberByDate(date) {
  ...
  const pastDays = (date - firstDay) / DAY_TIMESTAMP;
  const additionalDays = firstDay.getDay() === 0 ? 7 : firstDay.getDay();
  const weekNumber = Math.ceil((pastDays + additionalDays) / 7);
...
function getQuarter(date) {
  const resultMonth = new Date(date).getMonth();
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  let start = new Date(period.start.split('-').reverse().join('-'));
  ...
  while (start <= end) {
    ...
    if (!isWorkingDay) {
      start = new Date(start.setDate(start.getDate() + countOffDays));
      isWorkingDay = true;
    }
  }

  return result.map((item) => item.slice(0, 10).split('-').reverse().join('-'));
}
...
function isLeapYear(date) {
  ...
  return (yearDate % 4 === 0 && yearDate % 100 !== 0) || yearDate % 400 === 0;
}
